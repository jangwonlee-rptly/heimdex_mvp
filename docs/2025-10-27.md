...
- **Storage abstraction:** `app/core/storage.py` provides filesystem-backed storage today with an S3 stub for future expansion.
...
- **Storage abstraction:** Only local filesystem backend is implemented; S3 path raises `NotImplementedError`.
